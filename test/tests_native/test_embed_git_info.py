...
class GitInfoReproducibilityTest(TestCase):
    def test_same_commit_ignores_remote_and_checkout_ref(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_git(clone_a, "remote", "set-url", "origin", "git@example.invalid:fork/specter-diy.git")
            run_git(clone_b, "checkout", "--detach", commit)
            run_git(clone_b, "remote", "set-url", "origin", "https://example.invalid/other/specter-diy.git")
    ...
    def test_explicit_overrides_are_embedded(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("REPOSITORY = 'https://example.org/specter-diy'", content)
